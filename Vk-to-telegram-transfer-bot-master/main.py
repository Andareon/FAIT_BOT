...
def checkAttachments(msg, idd, userName):
    if not (msg.get('attachments')):
        return False
    transferAttachmentsToTelegram(idd, getAttachments(msg), userName)
    return True
...
def getFwdMessages(msg, idd):
    ...
    while not fwdMsg is None:
        ...
        checkAttachments(fwdMsg[0], idd, userName)
...
def checkRedirect_vk(msg):
    ...
    if not config.getCell("vk_" + chatid) is None:
        ...
        if forwardMessage is None:
            checkAttachments(msg['last_message'], chatid, userName)
...
def transferMessagesToTelegram(idd, userName, mbody, fwdList):
    ...
    if not fwdList is None:
        ...
        for f in fwdList[0:]:
            ...
            insert_table(time, f.get('userName'), f.get('body'))
        ...
        module.bot.send_message(config.getCell('vk_' + idd), niceText)
    if mbody:
        insert_table(time, userName, mbody)
...
def transferAttachmentsToTelegram(idd, attachments, userName):
    for j in attachments[0:]:
        ...
        if attType == 'photo' or attType == 'sticker':
            ...
            insert_table(time, userName, link)
            ...
            insert_table(time, userName, '( Неизвестный тип аттачмента )')
...
def input_vk():
    while True:
        ...
        try:
            ...
            rawMessages = module.vk.messages.getConversations(filter='unread', count=config.getCell('vk_msgForPick'))[
                'items']
            if not rawMessages:
                continue
